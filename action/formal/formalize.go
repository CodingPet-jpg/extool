...
func GetStepNoHandler() func(file *excelize.File, row []string, rowNum int) int {
	var expectStepNo = 1
	const (
		columnStepNoS = "C"
		columnStepNoI = 2
	)
	var R = func(file *excelize.File, row []string, rowNum int) int {
		if len(row) <= columnStepNoI+1 {
			...
			if i == expectStepNo {
				...
				err := file.SetCellInt("Sheet1", axis, expectStepNo)
				if err == nil {
					expectStepNo++
				}
			}
